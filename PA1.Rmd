...
Now use 'wDays' to determine if a measure of the dataset 'sumStepsPerDayNoNA' is on a weekend or on a weekday and add this with a new column 'wDay'.

```{r}
meanStepsPerIntervalNoNA <- aggregate(dfNoNA[,c('steps')], by=list(dfNoNA$interval), FUN=mean, na.rm=TRUE)
ticks <- meanStepsPerIntervalNoNA[seq(1, nrow(meanStepsPerIntervalNoNA), 24),][,1]
sumStepsPerDayNoNA$wDay <- with(df, ifelse(sumStepsPerDay$day %in% wDays, "weekday", "weekend"))
```